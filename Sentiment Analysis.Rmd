...
setwd("E:\\下载\\project\\GOT\\script\\S3")
```
...
```{r}
#count the frequency of each sentiment in a line and reshape the data set
...
```{r}
#check what words in the script contribute most to "trust" and see the frequency
tidy_script %>%
  inner_join(get_sentiments("nrc")) %>%
...
```{r}
#add into the stop words to see how the frequency of "positive" words changes
tidy_script %>%
...
```{r}
#check other sentiments such as "anger"
tidy_script %>%
  anti_join(custom_stop_words) %>%
  inner_join(get_sentiments("nrc")) %>%
...
tidy_script %>%
  ...
  mutate(index = line %/% 5) %>%
  ggplot(aes(x=index, y=n, color=sentiment)) %>%
  + geom_col(aes(fill=sentiment),col="#C0C0C0")+scale_fill_brewer(palette="Spectral")+theme(
